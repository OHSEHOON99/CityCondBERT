...
# ----------------------------------------------------------------
# forward (masked logits only) + accuracy + (옵션) heavy metrics
#   * 분해(splitting/bucketing)는 여기서 1회 수행하고 반환
#   * labels_dev 반환 제거 (불필요)
# ----------------------------------------------------------------
def _forward_pass_with_metrics(
    ...
    city_ids          = city_ids.to(device, non_blocking=True)       # ✅ added

    # ---- forward (masked logits only) ----
    logits_masked, mask_pos = model(
        input_seq_feature,
        attention_mask,
        labels=location_labels,
        city_ids=city_ids,                                            # ✅ added
    )

    # ---- accuracy on masked positions (lightweight) ----
    ...
    split_art = {
        ...
        "y_list": y_list,
        "buckets": buckets,
        "pred_masked": pred_list[0] if len(pred_list) > 0 else None,  # optional
        "pred_list": pred_list,
    }
...
def train_step(model, optimizer, criterion,
               input_seq_feature, location_labels, attention_mask, city_ids,   # ✅ city_ids 추가
...
def train_model(model, optimizer, train_loader, val_loader, num_epochs, device, run_dir, cfg,
    ...
    amp_dtype = torch.float16                      # <- always fp16
    scaler_enabled = use_amp                       # <- enable scaler only when CUDA+use_amp
    ...
    for epoch in range(num_epochs):
        ...
        for _, batch in train_prog:
            ...
            feats, labels, attn_mask, _, city_ids = batch     # <-- city_ids unpacked
            out = train_step(model, optimizer, criterion,
                             feats, labels, attn_mask, city_ids,  # <-- pass city_ids
        ...
        for _, batch in val_prog:
            feats, labels, attn_mask, uids, city_ids = batch       # <-- city_ids unpacked
            out = val_step(model, criterion,
                           feats, labels, attn_mask, city_ids,     # <-- pass city_ids
        ...
        if no_improve_count >= patience:
            print(f"[EarlyStopping] No val/GEO-BLEU improvement for 5 evals. Stop at epoch {epoch+1}.")
            break
